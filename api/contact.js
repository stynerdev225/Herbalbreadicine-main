...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Use API key directly (secure in Vercel's serverless environment)
    ...
    // Send to Google Sheets (optional, won't block the response)
    try {
      console.log('Sending to Google Sheets...');
      
      // Use node-fetch as a fallback if global fetch is not available
      const fetchToUse = typeof fetch !== 'undefined' ? fetch : require('node-fetch');
      
      // Use Promise to prevent this from blocking the response
      Promise.resolve().then(async () => {
        try {
          const sheetResponse = await fetchToUse('https://script.google.com/macros/s/AKfycbzfC-CUC4DNh4k28KFMhdgSDiyltVykWhijP6nLvoJqTdjbczb77W2n8kTHWO1fLotN/exec', {
            method: 'POST',
            headers: {
